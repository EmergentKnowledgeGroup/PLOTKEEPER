        ...
        with tempfile.TemporaryDirectory(dir=base) as folder:
            ...
            with mock.patch.object(GUARD, "git_head", return_value="f" * 40), mock.patch.dict("os.environ", {"PLOTKEEPER_REVIEW_KEY_FILE": str(key_path)}):
                ...
                receipt = {
                    ...
                    "candidate_sha": "f" * 40,
                    "reviewer": {"independent": True, "implemented_candidate": False, "delegated_candidate": False, "approved_candidate": False}
                }
                receipt["review_receipt_hash"] = hashlib.sha256(json.dumps(receipt, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()
                receipt_path = reviews / "deploy.json"
                receipt_path.write_text(json.dumps(receipt), encoding="utf-8")
                signature = hmac.new(b"test-review-key", receipt["review_receipt_hash"].encode(), hashlib.sha256).hexdigest()
                receipt_path.with_suffix(".json.sig").write_text(signature, encoding="utf-8")
