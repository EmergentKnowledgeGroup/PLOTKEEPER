...
Automatic entry points bind to `127.0.0.1` by default using one persisted high
private connector chosen during installation. A fixed port is never silently
taken over; owned-listener checks remain the safety boundary. There is no
authentication or TLS layer. An explicit non-loopback host exposes the service,
so it requires an appropriate access-control layer.
